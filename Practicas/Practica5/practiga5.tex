...
\end{lstlisting}
 ...
 Especificacion del ciclo:
	...
	\begin{itemize}
		...
		Supongo que vale $(s[j+1]>s[i])$ y entonces vale lo restante por razones muy fuleras que no quiero demostrar .
		\item $ (0\leq i <|s|\wedge 0\leq j+1\leq |s|)\wedge_L$
		...
		Supongo que vale $\neg (s[j+1]>s[i])$ y entonces vale lo restante por razones muy fuleras que no quiero demostrar.\smallskip \\
		Como $(I\wedge B) \Rightarrow wp(ciclo,I)$, podemos afirmar que el cuerpo del ciclo preserva el invariante.
	\end{itemize}
\end{enumerate}
