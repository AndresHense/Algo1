...
\end{lstlisting}
 ...
 Especificacion del ciclo:
	...
	\begin{itemize}
		...
		\item  $result=\sum_{i=0}^{j}(\textrm{if}\ i\ mod\ 2=1\ \textrm{then}\ s[i]\ \textrm{else}\ 0\ \textrm{fi})$\\
		Trivial, por $P_c$ $result=0$, y  como $j=0$, entonces
		 $\sum_{i=0}^{0}(\textrm{if}\ i\ mod\ 2=1\ \textrm{then}\ s[i]\ \textrm{else}\ 0\ \textrm{fi})=result=0$.
	\end{itemize}	 
	$\scalebox{1.3}{$(I\wedge \neg B)\Rightarrow Q_c$}$\smallskip \\
	Queremos demostrar que vale $Q_c$, asumiendo que vale  $I\wedge \neg B$.
	\begin{itemize}
